...
class PulsePalObject(object):
    ...
    def __init__(self, port_name):
        ...
        Args:
            port_name (str): The name of the USB serial port to which the Pulse Pal is
            connected (e.g. COM3 on Windows)

        Raises:
            PulsePalError: If there is a problem initializing the connection.
        """
        self.Port = ArCom(port_name, 12000000)
        # ArCom (Arduino Communication) wraps PySerial to simplify data
        # transactions with Arduino
        self._model = 0
        ...
        if (
            firmware_version < 20
        ):  # Model can be inferred from firmware. Model 1 spanned firmware v1-19.
            self._model = 1
        ...
        if self.firmware_version == 20:
            ...
            print("It fixes a bug in Pulse Gated trigger mode")
            print("when used with multiple inputs.")
            print("To update, follow the instructions at")
            print("https://sites.google.com/site/pulsepalwiki/updating-firmware")
        self.Port.write(
            (self.OP_MENU_BYTE, 89, 80, 89, 84, 72, 79, 78), "uint8"
        ...
        self.syncAllParams()  # Sets all parameters on the device to the current class

    def set2DefaultParams(self):
        """
        Returns all params to the defaults. This is called by the constructor, and may
        be subsequently called by the user (e.g. before running a new experiment)
        """
    ...
    def setFixedVoltage(self, channel, voltage):
        ...
        if self._model == 1:
            ...
            self.Port.write(
                (self.OP_MENU_BYTE, 79, channel), "uint8", voltage_bits, "uint16"
            )
        ok = self.Port.read(1, "uint8")  # Receive acknowledgement
        if len(ok) == 0:
            error = "Error: Pulse Pal did not return an acknowledgement "
            error += "after a call to setFixedVoltage()."
            raise PulsePalError(error)
    ...
    def programOutputChannelParam(self, param_name, channel, value):
        ...
        Args:
            param_name (str): The name of the parameter to program. Names are given
            above in self.outputParameterNames.
            channel (int): The output channel number to program the parameter for (1-4)
            value: The value to set for the parameter. Units: Voltage (if volts),
            Seconds (if time), Integer (if item)
        ...
        if 2 <= param_code <= 3 or param_code == 17:
            ...
            if self._model == 1:
                self.Port.write(
                    (self.OP_MENU_BYTE, 74, param_code, channel, value), "uint8"
                )
            else:
                self.Port.write(
        ...
        if len(ok) == 0:
            error = "Error: Pulse Pal did not return an acknowledgement "
            error += "after a call to programOutputChannelParam()."
            raise PulsePalError(error)
    ...
    def programTriggerChannelParam(self, param_name, channel, value):
        ...
        Args:
            param_name (str): The name of the parameter to program. Names are given
            above in self.triggerParameterNames.
        ...
        if len(ok) == 0:
            error = "Error: Pulse Pal did not return an acknowledgement "
            error += "after a call to programTriggerChannelParam()."
            raise PulsePalError(error)
    ...
    def syncAllParams(self):
        ...
        Raises:
            PulsePalError: If the device does not acknowledge the synchronization
            command.
        """
        # Preallocate
        program_values_16 = []
        ...
        # Prepare 16-bit voltages for Pulse Pal v2 (Pulse Pal v1 has 8-bit voltages
        # set with other 8-bit params below)
        if self._model == 2:
            pos = 0
            for i in range(1, 5):
        ...
        if len(ok) == 0:
            error = "Error: Pulse Pal did not return an acknowledgement "
            error += "after a call to syncAllParams()."
            raise PulsePalError(error)
    ...
    def sendCustomPulseTrain(self, custom_train_id, pulse_times, pulse_voltages):
        ...
        Args:
            ...
            pulse_times (list of float): The times at which each pulse should occur.
            Units = seconds.
        ...
        for i in range(0, n_pulses):
            pulse_times[i] = self._seconds2Cycles(
                pulse_times[i]
            )  # Convert seconds to multiples of update cycle (100us)
            pulse_voltages[i] = self._volts2Bits(pulse_voltages[i])

        op_code = (
            custom_train_id + 74
        )  # Serial op codes are 75 if custom train 1, 76 if 2
        if self._model == 1:
            self.Port.write(
                (self.OP_MENU_BYTE, op_code, 0),
                "uint8",
                n_pulses,
                "uint32",
                pulse_times,
        ...
        if len(ok) == 0:
            error = "Error: Pulse Pal did not return an acknowledgement "
            error += "after a call to sendCustomPulseTrain()."
            raise PulsePalError(error)

    def sendCustomWaveform(
        self, custom_train_id, pulse_width, pulse_voltages
        ...
        Args:
            ...
            pulse_width (float): The width of each pulse in the waveform.
            Units = seconds.
            pulse_voltages (list of float): The voltages for each pulse in the
        ...
        pulse_width_cycles = self._seconds2Cycles(
            pulse_width
        ...
        if len(ok) == 0:
            error = "Error: Pulse Pal did not return an acknowledgement "
            error += "after a call to sendCustomWaveform()."
            raise PulsePalError(error)
    ...
    def setContinuousLoop(self, channel, state):
        ...
        Args:
            channel (int): The output channel number to set the continuous
            loop state (1-4)
    ...
    def setRampEnabled(self, channel: int, enabled: bool):
        """
        Enable/disable ramp-down for a specific output channel (opcode 91).
        channel: 1..4
        """
        ...
        self.Port.write(
            (self.OP_MENU_BYTE, self.RAMP_SET_ENABLED_OPCODE, channel, val), "uint8"
        )
        ok = self.Port.read(1, "uint8")
        if len(ok) == 0:
    ...
    def setRampDuration(self, channel: int, s: int):
        ...
        ok = self.Port.read(1, "uint8")
        if len(ok) == 0:
            raise PulsePalError("No ACK after setRampDuration().")

    # microSD helpers (firmware op-code 90)
    def saveSDSettings(self, file_name: str) -> None:
        if self._model == 1:
            raise PulsePalError("no microSD in Pulse Pal 1.X.")
        payload = self._encode_filename(file_name)
        # 213, 90, 1 (=save), len, bytes
        self.Port.write(
            (self.OP_MENU_BYTE, 90, 1, len(payload)), "uint8", list(payload), "uint8"
        )

    def loadSDSettings(self, file_name: str, *, update_local: bool = False) -> bool:
        if self._model == 1:
            raise PulsePalError("no microSD in Pulse Pal 1.X.")
        payload = self._encode_filename(file_name)
        self.Port.write(
            (self.OP_MENU_BYTE, 90, 2, len(payload)), "uint8", list(payload), "uint8"
        )

        if not update_local:
            return True
        try:
            t32 = self.Port.read(32, "uint32")
            t16 = self.Port.read(12, "uint16")
            t8 = self.Port.read(26, "uint8")
        except Exception:
            return False

        idx = 0
        self.phase1Duration = [float("nan")] + [
            float(t32[idx + i]) / float(self.CYCLE_FREQUENCY) for i in range(0, 32, 8)
        ]
        self.interPhaseInterval = [float("nan")] + [
            float(t32[idx + i + 1]) / float(self.CYCLE_FREQUENCY)
            for i in range(0, 32, 8)
        ]
        self.phase2Duration = [float("nan")] + [
            float(t32[idx + i + 2]) / float(self.CYCLE_FREQUENCY)
            for i in range(0, 32, 8)
        ]
        self.interPulseInterval = [float("nan")] + [
            float(t32[idx + i + 3]) / float(self.CYCLE_FREQUENCY)
            for i in range(0, 32, 8)
        ]
        self.burstDuration = [float("nan")] + [
            float(t32[idx + i + 4]) / float(self.CYCLE_FREQUENCY)
            for i in range(0, 32, 8)
        ]
        self.interBurstInterval = [float("nan")] + [
            float(t32[idx + i + 5]) / float(self.CYCLE_FREQUENCY)
            for i in range(0, 32, 8)
        ]
        self.pulseTrainDuration = [float("nan")] + [
            float(t32[idx + i + 6]) / float(self.CYCLE_FREQUENCY)
            for i in range(0, 32, 8)
        ]
        self.pulseTrainDelay = [float("nan")] + [
            float(t32[idx + i + 7]) / float(self.CYCLE_FREQUENCY)
            for i in range(0, 32, 8)
        ]

        def bits_to_volts(bits):  # 16-bit span −10..+10 V
            return (float(bits) / 65535.0) * 20.0 - 10.0

        self.phase1Voltage = [float("nan")] + [
            bits_to_volts(t16[i]) for i in range(0, 12, 3)
        ]
        self.phase2Voltage = [float("nan")] + [
            bits_to_volts(t16[i + 1]) for i in range(0, 12, 3)
        ]
        self.restingVoltage = [float("nan")] + [
            bits_to_volts(t16[i + 2]) for i in range(0, 12, 3)
        ]

        # 4 channels x (isBiphasic, customTrainID, customTrainTarget, customTrainLoop)
        self.isBiphasic = [float("nan")] + [int(t8[i * 4 + 0]) for i in range(4)]
        self.customTrainID = [float("nan")] + [int(t8[i * 4 + 1]) for i in range(4)]
        self.customTrainTarget = [float("nan")] + [int(t8[i * 4 + 2]) for i in range(4)]
        self.customTrainLoop = [float("nan")] + [int(t8[i * 4 + 3]) for i in range(4)]

        # Trigger links (2 filas x 4 cols)
        base = 16
        self.linkTriggerChannel1 = [float("nan")] + [
            int(t8[base + 0 * 4 + j]) for j in range(4)
        ]
        self.linkTriggerChannel2 = [float("nan")] + [
            int(t8[base + 1 * 4 + j]) for j in range(4)
        ]
        # Trigger modes
        self.triggerMode = [float("nan"), int(t8[base + 8]), int(t8[base + 9])]

        return True

    def deleteSDSettings(self, file_name: str) -> None:
        """
        Deletes a .pps file from the microSD.
        """
        if self._model == 1:
            raise PulsePalError("Pulse Pal 1.X does not have microSD.")
        payload = self._encode_filename(file_name)
        self.Port.write(
            (self.OP_MENU_BYTE, 90, 3, len(payload)), "uint8", list(payload), "uint8"
        )

    # helper to normalize and encode a filename for microSD ops
    def _encode_filename(self, file_name: str) -> bytes:
        name = file_name.strip()
        if not name.lower().endswith(".pps"):
            name += ".pps"
        try:
            b = name.encode("ascii")  # firmware expects pure ASCII
        except UnicodeEncodeError as e:
            raise PulsePalError("Filename must be ASCII.") from e
        if len(b) > 255:
            raise PulsePalError("Filename too long (max 255 bytes).")
        return b
    ...
    def _volts2Bits(self, value):
        """
        Converts a voltage value in range -10V to +10V to its corresponding bit
        value for the 12-bit DAC.
        ...
        """
        return math.ceil(
            ((self._toDecimal(value) + 10) / self._toDecimal(20)) * self._dac_bitMax
        )

    def _seconds2Cycles(self, value):
        """
        Converts a time value in seconds to the corresponding number of refresh cycles
        for the Pulse Pal device.
    ...
    def __del__(self):
        """
        Destructor method that ensures the Pulse Pal connection is closed if the
        object is deleted.
        """
