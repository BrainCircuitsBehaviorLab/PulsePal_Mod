...
Modifications copyright (C) 2025 BRAIN CIRCUITS AND BEHAVIOR LAB
...
volatile uint32_t RampDurationUs[4] = { 1000000UL, 1000000UL, 1000000UL, 1000000UL };  // 1 s by default (adjustable by command)
  ...
  if (SerialUSB.available()) {         // If bytes are available in the serial port buffer
    ...
    if (CommandByte == OpMenuByte) {   // The first byte must be 213. Now, read the actual command byte. (Reduces interference from port scanning applications)
      ...
      switch (CommandByte) {
        ...
        case 92:
          {                              // Set Ramp Duration (ms) per channel
            byte ch = SerialReadByte();  // 1-4
            ...
            if (RampTotalTicks[ch - 1] == 0) RampTotalTicks[ch - 1] = 1;
            SerialUSB.write(1);
          }
          break;
      }
...
void LoadDefaultParameters() {
  ...
  for (int ch = 0; ch < 4; ch++) {
    ...
    if (RampTotalTicks[ch] == 0) RampTotalTicks[ch] = 1;
  }
...
byte RestoreParametersFromSD() {
  ...
  for (int chan = 0; chan < 4; chan++) {
    ...
    if (RampDurationUs[chan] == 0) RampDurationUs[chan] = 1;
    RampTotalTicks[chan] = RampDurationUs[chan] / TIMER_PERIOD_US;
    if (RampTotalTicks[chan] == 0) RampTotalTicks[chan] = 1;
  }
