...
char CommanderString[16] = " PULSE PAL Mod";         // Displayed at the menu top when disconnected from software
char DefaultCommanderString[16] = " PULSE PAL Mod";  // The CommanderString can be overwritten. This stores the original.
...
volatile bool RampEnabled = false;             // ON/OFF
volatile uint32_t RampDurationUs = 1000000UL;  // 1 s by default (adjustable by command)
volatile uint32_t RampTotalTicks = RampDurationUs / TIMER_PERIOD_US;

// Estado de ramp para cada canal
volatile bool RampActive[4] = { false, false, false, false };
volatile int32_t RampCurrent[4] = { 0, 0, 0, 0 };     // Current DAC value
volatile int32_t RampTarget[4] = { 0, 0, 0, 0 };      // Target value (usually RestingVoltage[ch])
volatile int32_t RampStep[4] = { 0, 0, 0, 0 };        // Increment per tick (can be negative)
...
bool buildCTName(const char* baseName, int trainId, char* out, size_t outSz) {
  ...
  size_t len = 0;
  while (baseName[len] != '\0' && len < outSz - 1) len++;

  size_t copyLen = (len < outSz - 1) ? len : (outSz - 1);
  for (size_t i = 0; i < copyLen; ++i) out[i] = baseName[i];
  out[copyLen] = '\0';
...
bool SaveCustomTrains2SD(const char* basePpsName) {
  ...
  for (int id = 1; id <= 2; ++id) {
    if (!buildCTName(basePpsName, id, ctName, sizeof(ctName))) return false;
...
bool LoadCustomTrainsFromSD(const char* basePpsName) {
  ...
  for (int id = 1; id <= 2; ++id) {
    if (!buildCTName(basePpsName, id, ctName, sizeof(ctName))) continue;
    ...
    if (n > 5001) n = 5001;  // prevent overflow
...
void setup() {
  ...
  // --- Try to load "settings.pps" first ---
  currentSettingsFileName = "settings.pps";
  ...
  if (settingsFile.open(currentSettingsFileNameChar, O_READ)) {
    ...
    if (validProgram == 252) {
      ...
      // settings.pps exists but invalid → fallback to default.pps
      settingsFile.close();
    ...
    // settings.pps not found → try default.pps
    currentSettingsFileName = "default.pps";
    currentSettingsFileName.toCharArray(currentSettingsFileNameChar,
                                        currentSettingsFileName.length() + 1);
    if (settingsFile.open(currentSettingsFileNameChar, O_READ)) {
      ...
      // Neither settings.pps nor default.pps found → hardcoded defaults
      LoadDefaultParameters();
    }
  }
...
inline void serviceRamps() {
  ...
  for (int ch = 0; ch < 4; ++ch) {
    ...
    // Compute next DAC value
    int32_t next = RampCurrent[ch] + RampStep[ch];

    // Clamp if we reached or passed the target
    if ((RampStep[ch] >= 0 && next >= RampTarget[ch]) || (RampStep[ch] <= 0 && next <= RampTarget[ch]) || (RampTicksLeft[ch] == 0)) {
      next = RampTarget[ch];
      RampActive[ch] = false;
    }
    ...
    if (CommandByte == OpMenuByte) {   // The first byte must be 213. Now, read the actual command byte. (Reduces interference from port scanning applications)
      ...
      switch (CommandByte) {
        case 72:
          ...
          break;
        case 73:
          {                                // Program the module - total program (can be faster than item-wise, if many parameters have changed)
        ...
        case 77:
          {  // Soft-trigger specific output channels. Which channels are indicated as bits of a single byte read.
            inByte2 = SerialReadByte();
            for (int i = 0; i < 4; i++) {
        ...
        case 78:
          ...
          break;
        case 79:
          {  // Write specific voltage to an output channel (not a pulse train)
          ...
          break;
        case 80:
          {  // Soft-abort ongoing stimulation without disconnecting from client
            for (int i = 0; i < 4; i++) {
              killChannel(i);
              DACFlags[i] = 1;
            }
        ...
        case 81:
          ...
          break;
        case 82:
          {                              // Set Continuous Loop mode (play the current parametric pulse train indefinitely)
          ...
          //        } break;
        case 85:
          {  // Return the currently loaded parameter file from the microSD card
            settingsFile.rewind();
            for (int i = 0; i < 178; i++) {
        ...
        case 86:
          {  // Override Arduino IO Lines (for development and debugging only - may disrupt normal function)
            inByte2 = SerialReadByte();
        ...
        case 87:
          {  // Direct Read IO Lines (for development and debugging only - may disrupt normal function)
            inByte2 = SerialReadByte();
          ...
          //        } break;
        case 89:
          {  // Receive new CommanderString (displayed on top line of OLED, i.e. "MATLAB connected"
            for (int x = 0; x < 6; x++) {
        ...
        case 90:
          {  // Save, load or delete the current microSD settings file
            byte confirmBit = 1;
        ...
        case 91:
          {  // RAMP_SET_ENABLED
            ...
            SerialUSB.write(1);  // ACK
          }
          break;

        case 92:
          {  // RAMP_SET_DURATION_MS
            // payload: uint32: duration in ms
            unsigned long ms = SerialReadLong();
            if (ms == 0) ms = 1;  // clamp to avoid 0
            RampDurationUs = ms * 1000UL;

            RampTotalTicks = RampDurationUs / TIMER_PERIOD_US;
            if (RampTotalTicks == 0) RampTotalTicks = 1;
    ...
    if (StimulusStatus[x] == 1) {  // if this output line has been triggered and is delivering a pulse train
      ...
      if (BurstStatus[x] == 1) {   // if this output line is currently gated "on"
        switch (PulseStatus[x]) {  // depending on the phase of the pulse
          case 0:
            {  // if this is the inter-pulse interval
              ...
              if ((CustomTrainID[x] == 0) || ((CustomTrainID[x] > 0) && (CustomTrainTarget[x] == 1))) {
                if (SystemTime == NextPulseTransitionTime[x]) {
                  ...
                  if (!((UsesBursts[x] == 1) && (NextPulseTransitionTime[x] >= NextBurstTransitionTime[x]))) {  // so that it doesn't start a pulse it can't finish due to burst end
                    ...
                    if ((CustomTrainID[x] > 0) && (CustomTrainTarget[x] == 1)) {
                      dacValue.uint16[x] = CustomVoltages[thisTrainIDIndex][CustomPulseTimeIndex[x]];
                      DACFlag = 1;
                      DACFlags[x] = 1;
                    } else {
                      dacValue.uint16[x] = Phase1Voltage[x];
          ...
          case 1:
            {  // if this is the first phase of the pulse
                  ...
                  if (InterPhaseInterval[x] == 0) {
                    ...
                    if (CustomTrainID[x] == 0) {
                      ...
                      if (CustomVoltages[thisTrainIDIndex][CustomPulseTimeIndex[x]] < 32768) {
                        dacValue.uint16[x] = 32768 + (32768 - CustomVoltages[thisTrainIDIndex][CustomPulseTimeIndex[x]]);
                        DACFlag = 1;
                        DACFlags[x] = 1;
                      } else {
                        dacValue.uint16[x] = 32768 - (CustomVoltages[thisTrainIDIndex][CustomPulseTimeIndex[x]] - 32768);
                        DACFlag = 1;
                        DACFlags[x] = 1;
                      }
                      if (CustomTrainTarget[x] == 0) {
                ...
                if (CustomTrainID[x] == 0) {
                  ...
                  if (CustomTrainID[x] == 1) {
                    if (CustomVoltages[0][CustomPulseTimeIndex[x]] < 32768) {
                      dacValue.uint16[x] = 32768 + (32768 - CustomVoltages[0][CustomPulseTimeIndex[x]]);
                      DACFlag = 1;
                      DACFlags[x] = 1;
                    } else {
                      dacValue.uint16[x] = 32768 - (CustomVoltages[0][CustomPulseTimeIndex[x]] - 32768);
                      DACFlag = 1;
                      DACFlags[x] = 1;
                    }
                  } else {
                    if (CustomVoltages[1][CustomPulseTimeIndex[x]] < 32768) {
                      dacValue.uint16[x] = 32768 + (32768 - CustomVoltages[1][CustomPulseTimeIndex[x]]);
                      DACFlag = 1;
                      DACFlags[x] = 1;
                    } else {
                      dacValue.uint16[x] = 32768 - (CustomVoltages[1][CustomPulseTimeIndex[x]] - 32768);
                      DACFlag = 1;
                      DACFlags[x] = 1;
                    }
                  }
        ...
        if (SystemTime == NextBurstTransitionTime[x]) {
          if (BurstStatus[x] == 1) {
            ...
            if ((CustomTrainID[x] > 0) && (CustomTrainTarget[x] == 1)) {
              if (CustomTrainID[x] == 1) {
                if (CustomPulseTimeIndex[x] < CustomTrainNpulses[0]) {
                  dacValue.uint16[x] = CustomVoltages[0][CustomPulseTimeIndex[x]];
                  DACFlag = 1;
                  DACFlags[x] = 1;
                }
              } else {
                if (CustomPulseTimeIndex[x] < CustomTrainNpulses[1]) {
                  dacValue.uint16[x] = CustomVoltages[1][CustomPulseTimeIndex[x]];
                  DACFlag = 1;
                  DACFlags[x] = 1;
                }
              }
...
void killChannel(byte outputChannel) {
  ...
  if (!RampEnabled || start == target || RampTotalTicks == 0) {
  ...
  // Compute step per tick (at least ±1)
  int32_t diff = target - start;
  int32_t step = diff / (int32_t)RampTotalTicks;
  if (step == 0) step = (diff > 0) ? 1 : -1;

  RampCurrent[outputChannel] = start;
  RampTarget[outputChannel] = target;
  RampStep[outputChannel] = step;
  RampTicksLeft[outputChannel] = RampTotalTicks;
    ...
    if (SSactive) {
      ...
      switch (inMenu) {
        ...
        case 2:
          {  // Channel menu
            switch (SelectedAction) {
              ...
              case 4:
                {
                  Phase1Duration[SelectedChannel - 1] = ReturnUserValue(1, 72000000, 1, 1);
                }
                break;  // phase 1 duration
              case 5:
                {
                  InterPhaseInterval[SelectedChannel - 1] = ReturnUserValue(1, 72000000, 1, 1);
                }
              ...
              case 7:
                {
                  Phase2Duration[SelectedChannel - 1] = ReturnUserValue(1, 72000000, 1, 1);
                }
                break;  // phase 2 duration
              case 8:
                {
                  InterPulseInterval[SelectedChannel - 1] = ReturnUserValue(1, 72000000, 1, 1);
                }
                break;  // pulse interval
              case 9:
                {
                  BurstDuration[SelectedChannel - 1] = ReturnUserValue(1, 72000000, 1, 1);
                }
                break;  // burst width
              case 10:
                {
                  BurstInterval[SelectedChannel - 1] = ReturnUserValue(1, 72000000, 1, 1);
                }
                break;  // burst interval
              case 11:
                {
                  PulseTrainDelay[SelectedChannel - 1] = ReturnUserValue(1, 72000000, 1, 1);
                }
                break;  // stimulus train delay
              case 12:
                {
                  PulseTrainDuration[SelectedChannel - 1] = ReturnUserValue(1, 72000000, 1, 1);
                }
          ...
          {  // Trigger menu
            switch (SelectedStimMode) {
              case 1:
                ...
                break;
              case 2:
                {  // Single example pulse. Timing for the example pulse is done with micros() instead of the HW timer.
        ...
        case 4:
          ...
          break;
        case 5:
          {  // Handle click in file load/save menu
            if (myFilePos < 1) {
              inMenu = 1;
              SelectedChannel = 8;
...
void RefreshActionMenu(int ThisAction) {
  switch (SelectedAction) {
    ...
    case 2:
      {
        write2Screen("<Biphasic Pulse>", FormatNumberForDisplay(IsBiphasic[SelectedChannel - 1], 3));
      }
      break;
    case 3:
      {
        write2Screen("<Phase1 Voltage>", FormatNumberForDisplay(Phase1Voltage[SelectedChannel - 1], 2));
      }
      break;
    case 4:
      {
        write2Screen("<Phase1Duration>", FormatNumberForDisplay(Phase1Duration[SelectedChannel - 1], 1));
      }
      break;
    case 5:
      {
        write2Screen("<InterPhaseTime>", FormatNumberForDisplay(InterPhaseInterval[SelectedChannel - 1], 1));
      }
      break;
    case 6:
      {
        write2Screen("<Phase2 Voltage>", FormatNumberForDisplay(Phase2Voltage[SelectedChannel - 1], 2));
      }
      break;
    case 7:
      {
        write2Screen("<Phase2Duration>", FormatNumberForDisplay(Phase2Duration[SelectedChannel - 1], 1));
      }
      break;
    case 8:
      {
        write2Screen("<Pulse Interval>", FormatNumberForDisplay(InterPulseInterval[SelectedChannel - 1], 1));
      }
      break;
    case 9:
      {
        write2Screen("<Burst Duration>", FormatNumberForDisplay(BurstDuration[SelectedChannel - 1], 1));
      }
      break;
    case 10:
      {
        write2Screen("<Burst Interval>", FormatNumberForDisplay(BurstInterval[SelectedChannel - 1], 1));
      }
      break;
    case 11:
      {
        write2Screen("< Train Delay  >", FormatNumberForDisplay(PulseTrainDelay[SelectedChannel - 1], 1));
      }
      break;
    case 12:
      {
        write2Screen("<Train Duration>", FormatNumberForDisplay(PulseTrainDuration[SelectedChannel - 1], 1));
      }
      break;
    case 13:
      {
        write2Screen("<Link Trigger 1>", FormatNumberForDisplay(TriggerAddress[0][SelectedChannel - 1], 3));
      }
      break;
    case 14:
      {
        write2Screen("<Link Trigger 2>", FormatNumberForDisplay(TriggerAddress[1][SelectedChannel - 1], 3));
      }
      break;
    case 15:
      {
        write2Screen("<Custom Train# >", FormatNumberForDisplay(CustomTrainID[SelectedChannel - 1], 0));
      }
      break;
    case 16:
      {
        write2Screen("<Custom Target >", FormatNumberForDisplay(CustomTrainTarget[SelectedChannel - 1], 4));
      }
      break;
    case 17:
      {
        write2Screen("<RestingVoltage>", FormatNumberForDisplay(RestingVoltage[SelectedChannel - 1], 2));
      }
      break;
    case 18:
      {
...
void RefreshTriggerMenu(int ThisAction) {
  switch (SelectedInputAction) {
    ...
    case 2:
      {
        write2Screen("< Trigger Mode >", FormatNumberForDisplay(TriggerMode[SelectedChannel - 1], 5));
      }
      break;
    case 3:
      {
...
void SaveCurrentProgram2SD() {
  ...
  settingsFile.write((byte)(RampEnabled ? 1 : 0));  // 1 byte
  // write RampDurationUs as little-endian uint32
  breakLong((unsigned long)RampDurationUs);
  writeLong2SD();
...
byte RestoreParametersFromSD() {
  ...
  // --- BACKWARD-COMPATIBLE TAIL ---
  uint32_t sz = settingsFile.fileSize();
  uint32_t pos = settingsFile.curPosition();
  uint32_t remaining = (sz >= pos) ? (sz - pos) : 0;

  byte isValidProgram = 0;

  if (remaining == 1) {
    // old file without ramp info
    isValidProgram = readByteFromSD();
  } else if (remaining >= 6) {
    // new file with ramp info
    byte rampEn = readByteFromSD();
    RampEnabled = (rampEn != 0);

    // read uint32 little-endian
    uint32_t dur = readLongFromSD();
    if (dur == 0) dur = 1;
    RampDurationUs = dur;
