...
# of numpy arrays to and from equivalent types on an Arduino-programmable
# microcontroller.
# A matching class for Arduino is provided in the ArCOM
# repository: https://github.com/sanworks/ArCOM
...
class ArCom(object):
    def __init__(self, serial_port_name, baud_rate):
        ...
        Args:
            serial_port_name (string) The name of the USB serial port as known
            to the OS
            Examples = 'COM3' (Windows), '/dev/ttyACM0' (Linux)
            baud_rate (uint32) The speed of the target USB serial device
            (bits/second)
        Returns:
            none
        """
        ...
        self.serialObject = serial.Serial(
            serial_port_name, baud_rate, timeout=10, rtscts=True
        )
    ...
    def write(self, *arg):
        ...
        Args:
            ...
            # First value:
                ...
                Arg 2. The datatype of the data in Arg 1 (must be supported,
                see self._typeBytes)
